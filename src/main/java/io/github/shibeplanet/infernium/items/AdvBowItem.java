package io.github.shibeplanet.infernium.items;
...
public class AdvBowItem extends ProjectileWeaponItem implements Vanishable {
    ...
    public AdvBowItem(double baseArrowDamage, float drawSpeed, Properties properties) {
        super(properties);
        this.baseArrowDamage = baseArrowDamage;
            ...
            if (!itemstack.isEmpty() || flag) {
                ...
                if (!((double)f < 0.1D)) {
                    ...
                    if (!p_40668_.isClientSide) {
                        ...
                        if (j > 0) {
                            abstractarrow.setBaseDamage(this.baseArrowDamage + (double)j * 0.5D + 0.5D);
                        } else {
                            abstractarrow.setBaseDamage(this.baseArrowDamage);
                        }
    ...
    public float getPowerForTime(int p_40662_) {
        float f = (float)p_40662_ / this.drawSpeed;
        f = (f * f + f * 2.0F) / 3.0F;
        if (f > 1.0F) {
            f = 1.0F;
        }
